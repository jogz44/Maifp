...
// Check if existing assistance exists
...
// Computed property for final amount due
...
// Computed property for total assistance amount in form
...
function cancelAssistance() {
  assistanceForm.value = {
    assistanceItems: [{ fundSource: 'MAIFIP', amount: 0, id: 1 }],
    remarks: '',
  }
...
async function applyAssistance() {
  ...
  try {
    ...
    if (isSuccess) {
      // Update patient data with new assistance
      patient.value.assistance = {
      ...
      $q.notify({
        type: 'positive',
        message: `Financial assistance totaling ${formatAmount(totalAssistanceAmount.value)} has been applied successfully!`,
        position: 'top',
      })
    } else {
